...
<script type="text/javascript">
	...
	//展示交易历史的方法
	function showPossibilityList(){
		$.ajax({
			url:"workbench/transaction/showPossibilityListByTranId.do",
			data:{
				"tranId":"${t.id}"
			},
			type:"get",
	...
	function changeIcon(stage,i){
		...
		if(currentPossibility=="0"){
			//如果当前阶段的可能性为0 ,则前面7个黑圈 后面一个红X一个黑X
			for (var i=0;i<point;i++){
				//---------黑圈
				$("#"+i).removeClass();//删除当前样式
				$("#"+i).addClass("glyphicon glyphicon-record mystage");//添加样式
				$("#"+i).css("color","#000000");//删除当前样式
			}
			for (var i=point;i<"<%=dvList.size()%>";i++){
				if (i==index){
					//当前阶段，红X
					$("#"+i).removeClass();//删除当前样式
					$("#"+i).addClass("glyphicon glyphicon-remove mystage");//添加样式
					$("#"+i).css("color","#ff0000");//删除当前样式
				}else {
					//不是当前阶段  黑X
					$("#"+i).removeClass();//删除当前样式
					$("#"+i).addClass("glyphicon glyphicon-remove mystage");//添加样式
			...
			for (var i=0;i<point;i++){
				if (i==index){
					//是当前阶段 绿标
					$("#"+i).removeClass();//删除当前样式
					$("#"+i).addClass("glyphicon glyphicon-map-marker mystage");//添加样式
					$("#"+i).css("color","#90F790");//删除当前样式
				}else if (i<index){
					//是当前阶段前面部分  绿√
					$("#"+i).removeClass();//删除当前样式
					$("#"+i).addClass("glyphicon glyphicon-ok-circle mystage");//添加样式
					$("#"+i).css("color","#90F790");//删除当前样式
				}else {
					//是当前阶段后面部分  黑圈
					$("#"+i).removeClass();//删除当前样式
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			if ("0".equals(currentPossibility)){
				//当前可能性为0 ，前面7个为黑圈 ， 后面两个其中一个黑X 一个红X

				//遍历每一个阶段，根据每一个阶段取得可能性
				for (int i=0;i<dvList.size();i++){
					DicValue dv = dvList.get(i);
				...
				//当前可能性不为0 ， 后面两个为黑X ， 前面7个 有 绿勾 绿标 黑圈
