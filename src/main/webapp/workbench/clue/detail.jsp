...
<script type="text/javascript">
	...
	$(function(){
		...
		//页面加载完成后，展示关联市场活动详细信息
		showActivityList();

		//点击关联市场活动
		$("#guanLian").click(function (){
			$("#aname").val("");
			getActivityListByNameNotByClueId();
			$("#bundModal").modal("show");
		})

		//点击关联市场打开模态窗口敲击查询框回车执行的方法
		$("#aname").keydown(function (event){
			if (event.keyCode==13){
				getActivityListByNameNotByClueId();

				//展现完列表后，记得将模态窗口默认回车行为禁用掉
				return false;
			}
		})
		//点击关联市场打开模态窗口点击关联按钮
	...
	function showActivityList(){
		$.ajax({
			...
			success:function (data){
				/*
				data
					[{市场活动1}{2}{3}]--》{id,name,startData,endDate,owner}-->id为tbl_clue_activity_relation的id
				 */
				var html = "";
				$.each(data,function (i,n){
					...
					html += '<td>'+n.owner+'</td>';
					html += '<td><a href="javascript:void(0);" onclick="deleteByClueId(\''+n.id+'\')"  style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
				})
	...
	//关联市场活动
	function getActivityListByNameNotByClueId(){
...
<%--						</tr>--%>
			...
			<div>
				<a href="javascript:void(0);" id="guanLian" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
